...
public class SimUtils {
    ...
    public static String getSimOperatorName(Context context, int slotIdx) {
        if (getSimStateBySlotIdx(context, slotIdx)) {
            Log.d("getSubidBySlotId","id :" +getSubidBySlotId(context, slotIdx) +"slotId :"+ slotIdx);
            return (String) getSimByMethod(context, SIM_OPERATOR_NAME, getSubidBySlotId(context, slotIdx));
        }
        return context.getResources().getString(R.string.no_sim_operator);
    }
    ...
    public static int getSlotIdFromSubid(Context context,int subid){
        try {
            SubscriptionManager subscriptionManager= (SubscriptionManager)context.getSystemService(Context.TELEPHONY_SUBSCRIPTION_SERVICE);
            Class<?>[] parameter = new Class[1];
            parameter[0] = int.class;
            Method getSoltIdMethod = subscriptionManager.getClass().getDeclaredMethod("getSlotId", parameter);
            if (null != getSoltIdMethod) {

                int slotid = (int)getSoltIdMethod.invoke(subscriptionManager, subid);
                return slotid;
            }
        } catch (Exception ex) {
    ...
    public static Boolean supporteSIM(){
        ...
        if(manager != null) {
            reader = manager.getCustomizationReader("System", HtcWrapCustomizationManager.READER_TYPE_XML, false);
        }
        if(reader != null) {
            return reader.readBoolean("supporteSIM",false);
        }
        return  false;
    }
