 ...
 */
public class TimerAlertReminderManager {
    private static final String TAG = "WorldClock.TimerAlertReminderManager";
    private static final boolean DEBUG_FLAG = HtcWrapHtcDebugFlag.Htc_DEBUG_flag;
    
    private static TimerAlertReminderManager mInstance;
    ...
    public static synchronized TimerAlertReminderManager getInstance() {
        if (mInstance == null) {
            mInstance = new TimerAlertReminderManager();
        }
        return mInstance;
    }
    
    private TimerAlertReminderManager() {
    }
    ...
    private void initReminderClient(final Context context, final boolean isLockScreen) {
        if (mHtcReminderClient == null) {
            mHtcReminderClient = new HtcReminderClient() {
                ...
                public void onViewModeChange(int viewMode) {
                    ...
                    if (viewMode == HtcReminderViewMode.TIMER_MODE) {
                        AlertUtils.timerAlert(context, AlertUtils.ALERT_DIALOG_NORMAL, isLockScreen);
//                        sendReminderReadyIntentToTimerService(context);
                    } else {
                        // TODO:
                        // If you aren't the current view mode.
                        // Please follow your design to do the proper handling.
                        if (viewMode <= HtcReminderViewMode.TIMER_MODE) { // priority is higher than timer
                            if (HtcReminderViewMode.TIMER_MODE == currentViewMode) {
                                if (DEBUG_FLAG) Log.d(TAG, "onViewModeChange: view mode's priority is higher than timer, dismiss it");
                ...
                public void unlock() {
                    ...
                    if (DEBUG_FLAG) Log.d(TAG, "unlock: currentViewMode = " + currentViewMode);
                    if (HtcReminderViewMode.TIMER_MODE != currentViewMode) {
     ...
     * Register View Mode
     */
   public void initRegisterViewMode(Context context, int viewMode, boolean isLockScreen) {
        initReminderClient(context, isLockScreen);
